...
def harry_news(a,b,c,d):
    ...
    tweets = api.user_timeline(screen_name=username, count=9, include_rts=False)
    for tweet in tweets:
        try:
            encoded_tweet = tweet.text.encode('utf-8')
            if 'white sox' in encoded_tweet.lower():
                retweet_link = 'https://twitter.com/ChicagoSports/status/' + str(tweet.id)
                retweet_news(a,b,c,d,retweet_link)
        except tweepy.TweepError as e:
            print(e.reason)
            f = open('log.txt', 'a')
            f.write('An exceptional hawkbot thing happened - %s' % e)
            f.close()
            sleep(9)
            continue
...
def retweet_news(a,b,c,d,retweet_link):
    options = ["""DJ, I read something interesting in the Chicago Tribune earlier today:
...
def flip_it(oppo_pitcher, oppo_roster, oppo_team):
    ...
    if will_it_run > 2:
        ...
        closers = [ "I think I'm saying that right. \n\n#Cubs",
                    "\n\n#Cubs",
                    "\n\n#Cubs",
                    "Does that sound like a city by Peoria? I'll call my good friend Pete Vonachen.\n#Cubs"]
...
while(True):
    today_game = mlbgame.day(currentYear, currentMonth, currentDay, home="Cubs", away = "Cubs")
    if today_game == []:
        break
    stats = today_game[0]
    ...
    if stats.game_status == "FINAL":
        ...
        people = ["", "", " and Tom Paciorek", " and 'Wimpy'", " and DJ", " and Darrin Jackson",
                  " and Milo Hamilton"]
        ...
        if stats.w_team == "White Sox":
            openers = ["#WhiteSox win!", "#WhiteSox win! Hell yes!", "Good guys win! #WhiteSox"]
            opener = random.choice(openers)

        else:
            openers = ["(30 seconds of silence)",
                       "Not a good day for the Sox." "White Sox lose today."]
            opener = random.choice(openers)

        my_status = """{} The score: The Sox {} and the {} {}. Here are your totals:\nFor the White Sox. {} {}, {} {} and {} {}.\nFor the {}, {} {}, {} {} and {} {}. \n{}. Goodnight, everybody. \n#WhiteSox #ChangeTheGame""".format(opener, cubs_runs, oppo_team, oppo_runs, cubs_runs,cub_r, cubs_hits, cub_h,
                                                                           cubs_errors, cub_e,
            ...
            if happened != atbat_string:
                if "homer" in atbat_string:
                    ...
                    if this_inning == 4 and stats.away_team == "White Sox" and this_half == "top":
                        harry_news(stats.home_team, stats.home_team_runs, stats.away_team, stats.away_team_runs)
